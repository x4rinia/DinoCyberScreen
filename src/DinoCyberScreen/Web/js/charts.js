...
class Sparkline {
  ...
  draw() {
    ...
    const c=this.ctx;c.clearRect(0,0,w,h); const color=getComputedStyle(document.querySelector('#app')).getPropertyValue(this.color).trim();
    const gradient=c.createLinearGradient(0,0,0,h);gradient.addColorStop(0,color+'24');gradient.addColorStop(1,color+'00');
    ...
    c.beginPath();this.values.forEach((v,i)=>{const x=i/(this.values.length-1)*w;const y=h-(v/100*h*.88+h*.06);i?c.lineTo(x,y):c.moveTo(x,y)});c.strokeStyle=color;c.lineWidth=dpr;c.shadowColor=color;c.shadowBlur=1.5*dpr;c.stroke();c.shadowBlur=0;
  }
}
